...
**Results**: A statistical significant relationship was found, a higher vaccination rate reduce the relative risk of COVID-19 by: 

Potential limitations of the analysis are due to the time and spatial autocorrelation in the data. \newline
**Conclusion**: Countries with current low vaccination rates should prioritize a policy to vaccinate their population with the goal to reduce the death toll of the COVID-19 pandemic.
...
The main hypothesis to test the effect of each vaccination level on the CFR for COVID-19, that is to test $H_0:\beta_1=0$ vs $H_A:\beta_1 \neq 0$. A likelihood ratio test is conduct to test whether the coefficient associated to vaccination status is significant. Furthermore, the coefficients are present along with their confidence intervals at 95% level. \newline
...
f_plot_coef <- function(mod,
  ...
  estimates %>% 
    ...
    coord_flip()+
    geom_hline(yintercept = 1, linetype="dashed")+
    labs(x=var_title, y=paste0("Estimated Coefficients: ",
...
location_coefficients %>% 
  ...
  coord_flip()+
  guides(col=F)+
  labs(x="WHO Region",col="",y="Estimated Coefficients: Country")
```
...
# nobs(model_random)
# ranef(model_random)
# exp(fixef(model_random)[2:4])
# exp(confint(model_random,2:4,method="Wald", level=0.95))
```
...
Conduct a dispersion test to see whether the data fitted to the model is more/less dispersed than expected. The test looks good, as we cannot reject the null hypothesis of over-dispersion.

```{r}
# Dispersion test
DHARMa::testDispersion(model.nb2) #looks good
```
...
model.nb3 <- MASS::glm.nb(deaths~location+month+offset(log(cases)), 
              data = df_model[c(-310,-641,-1291,-1337),]) 
...
lmtest::lrtest(model.nb1,model.nb2)

# plot(model_noOutliers)
# plot(model_poisson)
# plot(model.10levels)
par(mfrow = c(1,1))
